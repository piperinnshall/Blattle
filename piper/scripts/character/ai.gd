...
class AttackState extends AIState:
	...
	# Simplified: return true if AI *wants* to perform the attack (proximity-based).
	func get_light_input(ai: FightingAI) -> bool:
		var player = ai.get_player()
		if not player:
			return false
		var distance = ai.global_position.distance_to(player.global_position)
		return distance <= ai.attack_range
	...
	func get_heavy_input(ai: FightingAI) -> bool:
		...
		if not player:
			return false
		var distance = ai.global_position.distance_to(player.global_position)
		return distance <= ai.attack_range and randf() < ai.heavy_attack_chance
...
class RetreatState extends AIState:
	...
	func get_dash_input(ai: FightingAI) -> bool:
		return retreat_timer < ai.retreat_duration * 0.3
...
var update_frequency: float = 0.1  # Update 10x per second
...
func _physics_process(delta):
	...
	# Update AI logic at fixed intervals
	update_timer += delta
